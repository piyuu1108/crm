    ...
    return await db.transaction(async (tx) => {
        ...
        const upsertedApps = await tx
            ...
            .onConflictDoUpdate({
                ...
                // THE SILVER BULLET: Skip update if nothing changed
                where: sql`${sessionApps.totalSeconds} IS DISTINCT FROM EXCLUDED.total_seconds 
                       OR ${sessionApps.activeSeconds} IS DISTINCT FROM EXCLUDED.active_seconds 
                       OR ${sessionApps.idleSeconds} IS DISTINCT FROM EXCLUDED.idle_seconds`
            })
        ...
        if (detailValues.length > 0) {
            const upsertedDetails = await tx
                ...
                .onConflictDoUpdate({
                    ...
                    // THE SILVER BULLET: Skip update if nothing changed
                    where: sql`${sessionDetails.totalSeconds} IS DISTINCT FROM EXCLUDED.total_seconds 
                           OR ${sessionDetails.activeSeconds} IS DISTINCT FROM EXCLUDED.active_seconds 
                           OR ${sessionDetails.idleSeconds} IS DISTINCT FROM EXCLUDED.idle_seconds`
                })
                .returning({
            ...
            for (let i = 0; i < segmentValues.length; i += CHUNK_SIZE) {
                ...
                await tx
                    ...
                    .onConflictDoUpdate({
                        ...
                        // THE SILVER BULLET: Prevent rewriting old, closed segments
                        where: sql`${activitySegments.endedAt} IS DISTINCT FROM EXCLUDED.ended_at`
                    });
            }
        }
